...
const AllLeaveRequests = ({ leaveRequests }) => {
    ...
    return (
        <div className='flex md:flex-row flex-col md:w-full md:flex-wrap gap-2 px-2'>
            {leaveRequests.length===0?<>No datat found</>:leaveRequests?.map((leave, id) => (
                <div key={id} className="md:w-[40%] w-full  bg-white p-4 flex gap-3 md:gap-6">
                    <div className="flex gap-4 md:w-[15%] w-[20%]">
                        <img src={leave?.employeeId?.profilePicture} alt="Employee" className="w-16 h-16 rounded-full" />
                    </div>
                    <div className='flex flex-col md:w-[85%] w-[80%] gap-3'>
                        ...
                        <div className="flex gap-5">
                            {leave.status === 'pending' && (
                                <>
                                    <button className="bg-green-200 h-9 md:w-36 w-28 flex justify-center items-center text-sm font-semibold font-inter p-2 rounded-lg" onClick={() => toggleInput(leave._id, 'approved')}>
                                        <span className='size-max rounded-full p-1 flex'><FcApproval className='size-6' /></span>
                                        <span className='text-xs'>Approve</span>
                                    </button>
                                    <button className="bg-red-100 h-9 md:w-36 w-28 flex justify-center items-center text-sm font-semibold font-inter gap-1 p-2 rounded-lg" onClick={() => toggleInput(leave._id, 'rejected')}>
                        ...
                        {showInput[leave._id] && (
                            <div className='flex flex-col gap-3'>
                                <input
                                    ...
                                    className='border-[1px] h-12 md:w-full w-[80%] border-gray-300 rounded-lg outline-none'
                                />
                                <div className='flex gap-5'>
                                    <button className="h-9 md:w-32 w-28 p-2 rounded-lg shadow-md text-white font-inter  bg-blue-500  ' " onClick={() => handleApprove(leave?._id)}>Submit</button>
                                    <button className="h-9 md:w-32 w-28  p-2 rounded-lg shadow-md text-white font-inter bg-yellow-500   " onClick={() => setShowInput(false)}>Cancel</button>
                                </div>
                            </div>
                        )}
                    </div>
